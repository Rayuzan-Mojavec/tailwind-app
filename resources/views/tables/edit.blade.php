...
@section('main-part')
    <div class="container w-3/5 bg-white shadow-lg my-32 px-8 py-16 mx-auto rounded-xl">
        <form method="post" action="{{ route('tables.update', '$people->ssn') }}">
            @csrf
            @method('put')
            <div class="flex flex-col justify-center mb-8">
                <label for="ssn" class="text-lg">SSN</label>
                <input type="text" class="form-input rounded-xl" id="ssn" name="ssn"
                    value="{{ old('ssn', $people->ssn) }}">
                @error('ssn')
            ...
            <div class="flex flex-col justify-center mb-8">
                ...
                <input type="text" class="form-input rounded-xl" id="namae" name="namae"
                    value="{{ old('namae') }}">
                @error('namae')
                    <div class="text-red-500 mt-1">
                        {{ $message }}
            ...
            <div class="flex flex-col justify-center mb-8">
                ...
                <select class="rounded-xl" id="gender" name="gender">
                    @foreach ($gender as $jendah)
                        @if (old('gender', "$people->gender") == $jendah)
            ...
            <div class="flex flex-col justify-center mb-8">
                ...
                <select class="rounded-xl" id="rerijyon" name="rerijyon">
                    @foreach ($religion as $rerijyon)
                        @if (old('rerijyon', "$people->rerijyon") == $rerijyon)
            ...
            <div class="flex flex-col justify-center mb-8">
                ...
                <input type="text" class="form-input rounded-xl" id="address" name="address"
                    value="{{ old('address', $people->address) }}">
                @error('address')
                    <div class="text-red-500 mt-1">
                        {{ $message }}
                    </div>
            ...
            <div class="flex flex-col justify-center mb-8">
                ...
                <input type="date" class="form-input rounded-xl" id="birth_date" name="birth_date"
